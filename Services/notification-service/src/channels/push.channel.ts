...
export class PushChannel extends BaseNotificationChannel {
  ...
  async send(
    ...
    try {
      ...
      if (
        ...
        console.warn(`Invalid/expired FCM token: ${recipient}`);
      }
  ...
  async sendToMultiple(
    ...
    try {
      if (tokens.length > 500) {
        console.warn(
          `Token count (${tokens.length}) exceeds limit. Splitting into batches.`,
        );
  ...
  private async sendInBatches(
    ...
    console.log(`Total: ${totalSuccess}/${tokens.length} notifications sent`);
  ...
  async subscribeToTopic(tokens: string[], topic: string): Promise<void> {
    ...
    try {
      ...
      if (response.failureCount > 0) {
        console.warn(`${response.failureCount} tokens failed to subscribe`);
      }
    } catch (error: any) {
