...
import { paymentGrpcClient } from "../grpc/clients/payment.client";
...
export const placeOrderService = async (
  ...
  for (const item of cart_items) {
    const product = db_products.find(
  ...
  expiration_date.setMinutes(
    expiration_date.getMinutes() + ORDER_EXPIRATION_MINUTES,
  );
  ...
  if (payment_method === "Card") {
    try {
      const { payment } = await paymentGrpcClient.createPayment(
        user_id,
        final_price * 100,
        "usd",
        "ROVEX Order",
        { orderId: order._id, items: String(order_items.length) },
      );
      payment_data = payment;
    } catch (error) {
      console.error("Payment Gateway Down:", (error as any).message);
      payment_error =
        "Payment system unavailable. Please retry from My Orders.";
    }
  }
...
export const releaseExpiredStockService = async () => {
  ...
  console.log(
    `Found ${expired_orders.length} expired orders. Releasing stock...`,
  );
...
export const retryPaymentService = async (
  ...
  try {
    ...
    throw new AppError(
      "Payment system still down. Please try again later.",
      503,
    );
  }
};
...
export const cancelOrderService = async (user_id: string, order_id: string) => {
  ...
  // Validation
  const cancellable_statuses = ["Pending", "PendingPayment", "Processing"];
...
export const getAllOrdersService = async (
  ...
  if (!query) {
    throw new AppError(
      "error generation query or you do not have an accesss please try again ",
      400,
    );
  }
...
export const updateOrderService = async (
  ...
  if (
    updateOrderBody.location &&
    order.order_status !== "Processing" &&
    order.order_status !== "PendingPayment"
  ) {
    throw new Error(
      `Cannot update location. Current status is '${order.order_status}', but it must be 'Processing' or 'PendingPayment'.`,
    );
  }
